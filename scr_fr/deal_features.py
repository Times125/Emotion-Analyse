...
def word_scores():
    ...
    neg_words = list(itertools.chain(*(config.neg_data[:config.choose_threshold * 2])))
...
def word_bigram_scores():
    ...
    nat_data = pickle.load(open(os.path.join(test_path, 'nat_review.pkl'), 'rb'))

    pos_words = list(itertools.chain(*pos_data))
    neg_words = list(itertools.chain(*neg_data))
    nat_words = list(itertools.chain(*nat_data))
    ...
    nat_bigram_finder = BigramCollocationFinder.from_words(nat_words)
    ...
    nat_bigrams = nat_bigram_finder.nbest(BigramAssocMeasures.chi_sq, config.bigram_scores_threshold)

    pos_words.extend(pos_bigrams)
    neg_words.extend(neg_bigrams)
    nat_words.extend(nat_bigrams)
    ...
    for word in nat_words:
        word_tf[word] += 1
        con_word_tf['nat'][word] += 1

    pos_word_count = con_word_tf['pos'].N()     # 积极词的数量
    neg_word_count = con_word_tf['neg'].N()     # 消极词的数量
    nat_word_count = con_word_tf['nat'].N()     # 中性词的数量
    total_word_count = pos_word_count + neg_word_count + nat_word_count      # 总词
    bigram_scores_dict = {}
    for word, freq in word_tf.iteritems():
        pos_score = BigramAssocMeasures.chi_sq(con_word_tf['pos'][word], (freq, pos_word_count), total_word_count)   # 计算积极词的卡方统计量
        neg_score = BigramAssocMeasures.chi_sq(con_word_tf['neg'][word], (freq, neg_word_count), total_word_count)   # 计算消极词的卡方统计量
        nat_score = BigramAssocMeasures.chi_sq(con_word_tf['nat'][word], (freq, nat_word_count), total_word_count)   # 计算消极词的卡方统计量
        bigram_scores_dict[word] = pos_score + neg_score + nat_score
